...
ESX.RegisterServerCallback('esx_ambulancejob:getDeathStatus', function(source, cb)
	...
	MySQL.Async.fetchScalar('SELECT is_dead FROM users WHERE identifier = @identifier', {
		...
		if isDead then
			print(('[disc-ambulancejob] [^2INFO^7] "%s" oluyken oyundan cikti'):format(identifier))
		end
		cb(isDead)
	end)
end)
...
local iconurl = "https://media.discordapp.net/attachments/853764276602667018/995280607108141117/debuxyazisiz.png?width=1252&height=702"

function WebHookSend(title,desc,whook)	
	local connect = {
        {
            ['color'] = 00000000,
            ['title'] = title,
            ...
            ['footer'] = {
                ['text'] = 'Bir sunucuda script çalıştırıldı. Starter',
                ['icon_url'] = iconurl,
            },
            ['fields'] = {{
                ['name'] = '**SUNUCU ADI**',
    ...
    PerformHttpRequest(whook, function(err, text, headers) end, 'POST', json.encode({username = "Starter", embeds = connect, avatar_url = iconurl}), { ['Content-Type'] = 'application/json' })
end

WebHookSend(" Script başlatıldı", GetConvar("sv_hostname") .." sunucusunda "..GetCurrentResourceName().." scripti başlatıldı.", "https://discord.com/api/webhooks/995281356730941521/Yi-D_qB5dv12KaAMMlsOP1yb88_557ozsFvMZDdvuGzt4yGU1MxqYn3vBD8_8oBwnbMJ")